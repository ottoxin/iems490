...
def main(data_path: str, out_path: str, prompt_path: str, limit: int | None = None):
    items = load_bbh_movie_recommendation(data_path)
    if limit is not None:
        items = items[:limit]
    template = Path(prompt_path).read_text(encoding="utf-8")
    rows = []
    for ex in tqdm(items, desc="Baseline"):
    ...
    print(f"Baseline accuracy: {acc:.3f}  (N={len(rows)})")
    print(f"Wrote: {out_path}")

if __name__ == "__main__":
    """
    Usage:
      python -m src.run_baseline data/movie_recommendation.json outputs/baseline.json prompts/baseline_movie_rec.txt
      # with limit
      python -m src.run_baseline data/movie_recommendation.json outputs/baseline_2.json prompts/baseline_movie_rec.txt 2
    """
